   	...
   	if ($_SESSION['username'] == "admin")
   	{
               ...
               while ($row = mysqli_fetch_array($result))
           {
                ...
                echo "</tr>";
   						...
   						//need this query before i close my table tag so everything is included
   						//COUNT ALL PARTICIPANTS QUERIES
   						$participants = "SELECT COUNT(surveyID) FROM simplesurvey";
               //variables that will store gender details
               $result = mysqli_query($connection, $participants);
               while ($row = mysqli_fetch_array($result))
               $people = $row['COUNT(surveyID)'];

   						$participants = "SELECT COUNT(q1) FROM simplesurvey";
               //variables that will store gender details
               $result = mysqli_query($connection, $participants);
               while ($row = mysqli_fetch_array($result))
               $t1 = $row['COUNT(q1)'];

   						$participants = "SELECT COUNT(q2) FROM simplesurvey";
               //variables that will store gender details
               $result = mysqli_query($connection, $participants);
               while ($row = mysqli_fetch_array($result))
               $t2 = $row['COUNT(q2)'];

   						$participants = "SELECT COUNT(q3) FROM simplesurvey";
               //variables that will store gender details
               $result = mysqli_query($connection, $participants);
               while ($row = mysqli_fetch_array($result))
               $t3 = $row['COUNT(q3)'];

   						$participants = "SELECT COUNT(q4) FROM simplesurvey";
               //variables that will store gender details
               $result = mysqli_query($connection, $participants);
               while ($row = mysqli_fetch_array($result))
               $t4 = $row['COUNT(q4)'];

   						$participants = "SELECT COUNT(q5) FROM simplesurvey";
               //variables that will store gender details
               $result = mysqli_query($connection, $participants);
               while ($row = mysqli_fetch_array($result))
               $t5 = $row['COUNT(q5)'];
   						...
   						echo "<tr><td>$people total responses</td>
   						<td>$t1 total responses</td>
   						<td>$t2 total responses</td>
   						<td>$t3 total responses</td>
   						<td>$t4 total responses</td>
   						<td>$t5 total responses</td>
   						</tr>";
   						echo "</table>";
               ...
               $car = $row['COUNT(q5)'];
   						...
   						//SUMMARISED Answer
   						echo "<p>A total of $people	people have taken part in this simple survey, where $answer1 are Man, $answer2 are female and $answer3 they would prefer not to say.</p>";
   						echo "<p>$car of these users have selected to have a preference with car, while $transport have chosen public transport, $Bike with bike and $Walk prefer walk.</p>";
